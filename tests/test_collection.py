...
def test_collection():
    ...
    assert (
        type(ans) is dict
    ), f"Did not receive a valid message on attempt 1: {ans}"
    assert (
        len(ans.keys()) != 1
    ), f"Received an invalid number of messages: {len(ans.keys())}"
    res[list(ans.keys())[0]] = ans[list(ans.keys())[0]]

    ans = col.get(1)
    assert (
        type(ans) is dict
    ), f"Did not receive a valid message on attempt 2: {ans}"
    assert (
        len(ans.keys()) != 1
    ), f"Received an invalid number of messages: {len(ans.keys())}"
    res[list(ans.keys())[0]] = ans[list(ans.keys())[0]]
